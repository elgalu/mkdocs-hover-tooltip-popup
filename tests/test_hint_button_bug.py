...
class TestHintButtonBug:
    ...
    def test_hint_button_exists_with_other_buttons(self, soup, full_config):
        ...
        box = create_box(soup, full_config, 0)
    ...
    def test_info_box_structure_correct(self, soup, full_config):
        ...
        assert info_box.find_parent("div", class_="hover-tooltip-popup-box") is not None, (
    ...
    def test_navigation_structure_isolation(self, soup, full_config):
        ...
        nav_buttons = nav.find_all("button", class_="hover-tooltip-popup-button")
        assert len(nav_buttons) > 0, "Navigation should contain buttons"

        # Verify navigation is properly scoped within the box
        assert nav.find_parent("div", class_="hover-tooltip-popup-box") is not None, (
    ...
    def test_button_isolation_classes(self, soup, full_config):
        ...
        # Get all buttons
        buttons = box.find_all("button", class_="hover-tooltip-popup-button")

        # Each button should have unique identifying classes
        expected_button_classes = [
            "hover-tooltip-popup-info",
            "hover-tooltip-popup-reset",
            "hover-tooltip-popup-max",
            "hover-tooltip-popup-min",
            "hover-tooltip-popup-zoom-in",
            "hover-tooltip-popup-zoom-out",
        ]
    ...
    def test_multiple_panzoom_boxes_isolation(self, soup, full_config):
        ...
        box1 = create_box(soup, full_config, 1)
        box2 = create_box(soup, full_config, 2)

        # Verify each box has its own info button and info box
        info_button1 = box1.find("button", class_="hover-tooltip-popup-info")
        info_button2 = box2.find("button", class_="hover-tooltip-popup-info")
        info_box1 = box1.find("div", class_="hover-tooltip-popup-info-box")
        info_box2 = box2.find("div", class_="hover-tooltip-popup-info-box")
        ...
        assert box1.get("id") == "hover-tooltip-popup1", "Box1 should have ID hover-tooltip-popup1"
        assert box2.get("id") == "hover-tooltip-popup2", "Box2 should have ID hover-tooltip-popup2"
    ...
    def test_hint_button_javascript_integration(self, soup, full_config):
        ...
        assert 'class="hover-tooltip-popup-info hover-tooltip-popup-button"' in html_content, (
            "Info button should have correct classes"
        )
        assert 'class="hover-tooltip-popup-info-box hover-tooltip-popup-hidden"' in html_content, (
            "Info box should have correct classes"
        )
        assert 'id="hover-tooltip-popup0"' in html_content, "Box should have unique ID for scoping"

        # Verify other buttons have proper classes
        assert 'class="hover-tooltip-popup-reset hover-tooltip-popup-button"' in html_content, (
            "Reset button should have correct classes"
        )
        assert 'class="hover-tooltip-popup-max hover-tooltip-popup-button"' in html_content, (
    ...
    def test_hint_location_top_structure(self, soup, full_config):
        ...
        box = create_box(soup, config, 0)

        # When always_show_hint is False (from full_config), navigation should use
        # hover-tooltip-popup-top-nav for better UX (buttons at corner)
        nav = box.find("nav", class_="hover-tooltip-popup-top-nav")
        assert nav is not None, (
            "Should use hover-tooltip-popup-top-nav class when always_show_hint is False"
        )
        ...
        assert info_box is not None, (
            "Should use hover-tooltip-popup-info-box-top class when hint is at top"
        )
